...
\begin{marginfigure}
    ...
    \label{fig:04_LBC}
\end{marginfigure}
To assess the effectiveness of the proposed minimum slenderness limit, we conducted a new round of optimization on the L-shaped beam described in \secref{sec:03_applications} and showed in \figref{fig:04_LBC}. The material and geometric data are presented in \tabref{tab:04_mat_l}. In the \figref{fig:04_tto_slend}, we present the optimized structures obtained using this modified formulation and the stress limits $\sigma_\text{L}$ values of 1, $0.8$, $0.3$, and $0.2$. The first two values have already been used and the results have been presented in \tabref{tab:03_TTO_results}. They highlighted the limits of Formulation \eqrefnotext{eq:03_optim_original} when imposing a specified slenderness limit ($\lambda<15$). The last two values are introduced here to test how the \ref{eq:04_slend_constraints} constraints affect the truss topology for extreme cases. 
\begin{margintable}
    \small
    \centering
    \begin{tabular}{cc}
