...
The test cases are optimized using the two-step resolution strategy proposed in \secref{sec:04_2step_opt} implemented with five calls of reinitialization (2S-5R) with $k_{\text{max}}=5$. The reinitialization magnitude parameter $\vect{\phi}$ is set up using the same parameters listed in \tabref{tab:04_param}, that leads to $\vect{\phi} = \left[ 0.8000, 0.6400, 0.4096, 0.1677, 0.0281 \right]$.
...
The second metric is $\psi$ and is defined as the mean value of the normalized mechanical failure criterion $\vect{c}_\text{f}$ weighted on the volumes of the individual bars:
\begin{equation}
...
\begin{marginfigure}
    ...
    \caption{\todo{add monolitic result with a line horizontal}}
...
\begin{table}
    ...
    this stuty suggest that we have here a compromise between volume (and then mass) and manufacturing complexity. 
